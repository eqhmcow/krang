...
=head2 3.10 - In development

=over

=item *

Added storage of C<krang_pager_curr_page_num> and C<krang_pager_show_big_view>
to session cache in In L<Krang::HTMLPager>. The page number is forced to C<1>
when switching between Retired or Live templates, media, and stories. [Brad Oaks]

=item *

Searching for media on the advanced tab will now return media of all types
when no media_type_id is selected. [Brad Oaks]
