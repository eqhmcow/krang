...
like(
    $@,
    qr(Can't locate object method "no_such_param" via package .*::Group),
    'Test invalid create param'
);
...
like(
    $@,
    qr(group_ids array ref may only contain numeric IDs),
    'Test bad array ref input to group_ids'
);
...
like(
    $@,
    qr(Invalid security level 'no_such_permission' for desk_id '666'),
    "Die on invalid desk security level"
);
...
like(
    $@,
    qr(Invalid security level 'no_such_permission' for category_id '777'),
    "Die on invalid category security level"
);
...
my $new_site            = pkg('Site')->new(
    preview_url  => $new_site_uniqueness . 'preview.com',
...
$new_site->save();
my ($new_cat_id) =
  pkg('Category')->find(site_id => $new_site->site_id, parent_id => undef, ids_only => 1);
($load_group) = pkg('Group')->find(name => $unique_group_name);
...
my $dbh                = dbh();
my ($perm_cache_count) = $dbh->selectrow_array(
    qq/ select count(*) from
                                                user_category_permission_cache
                                                where category_id=? /,
    {RaiseError => 1}, $new_cat_id
);
is($perm_cache_count, $user_count,
    "Permissions cache created for new category for $user_count users");
...
($perm_cache_count) = $dbh->selectrow_array(
    qq/ select count(*) from
                                                user_category_permission_cache
                                                where category_id=? /,
    {RaiseError => 1}, $new_cat_id
...
is(pkg('Group')->user_desk_permissions($desk_perm_test_desk_id),
    "hide", "Admin has 'hide' desk access");
...
$admin_user->save();
is(pkg('Group')->user_desk_permissions($desk_perm_test_desk_id),
    "read-only", "Admin has 'read-only' desk access");
...
$admin_group->save();
is(pkg('Group')->user_desk_permissions($desk_perm_test_desk_id),
    "edit", "Admin has 'edit' desk access");
...
is(pkg('Group')->user_asset_permissions("media"), "hide", "Admin has 'hide' access to media");
...
is(pkg('Group')->user_asset_permissions("media"),
    "read-only", "Admin has 'read-only' access to media");

# Change permissions for admin group, for media, to "edit".  Re-check -- should be "edit" now.
$admin_group->asset_media("edit");
$admin_group->save();
is(pkg('Group')->user_asset_permissions("media"), "edit", "Admin has 'edit' access to media");
...
is(
    $user_admin_permissions{"admin_delete"},
...
is(pkg('Group')->user_admin_permissions("may_publish"), 0, "Admin has may_publish=>0 access");

# Add new group to Admin user and check again.  Access should now be "read-only"
($admin_user) = pkg('User')->find(login => 'system', hidden => 1);
$admin_user->group_ids_push($admin_perm_test_group->group_id);
$admin_user->save();
is(pkg('Group')->user_admin_permissions("may_publish"), 1, "Admin has may_publish=>1 access");
...
# second level hidden.  All categories below should be hidden too.
{
    ...
    my $one = pkg('Category')->new(
        dir       => 'one',
        parent_id => $root->category_id
    );
    $one->save();
    my $two = pkg('Category')->new(
        dir       => 'two',
        parent_id => $one->category_id
    );
    $two->save();
    my $three = pkg('Category')->new(
        dir       => 'three',
        parent_id => $two->category_id
    );
    $three->save();
    my $four = pkg('Category')->new(
        dir       => 'four',
    ...
    ok(not(grep { $_->category_id == $one->category_id } @cats), '/one should be hidden');
    ok(not(grep { $_->category_id == $two->category_id } @cats), '/one/two should be hidden');
