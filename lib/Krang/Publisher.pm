...
# _assemble_pages()
#
# @pages = $self->_assemble_pages(story    => $story,
#                                 category => $category
...
sub _assemble_pages {
    ...
    if ($args{mode} eq 'preview') { $self->{is_preview} = 1; $self->{is_publish} = 0; }
    elsif ($args{mode} eq 'publish') { $self->{is_preview} = 0; $self->{is_publish} = 1; }
    else {
        croak __PACKAGE__ . ": mode '$args{mode}' not valid.\n";
    }
