...
=head2 3.05 - IN DEVELOPMENT

=over

=item *

Fixed bug in L<Krang::Handler>: Make sure, each application request
has a window_id, even if the URL was entered manually in a newly
opened tab/window after the user has already been authenticated. [Bodo
Schulze]
...
Upgraded: Apache from 1.3.37 to 1.3.41, mm from 1.4.2 to 1.4.2, and mod_ssl 
from 2.8.28 to 2.8.31 to incorporate latest upstream security bug fixes; 
Disabled use of HTTP TRACE method in krang httpd and siteserver conf templates 
to address a security vulnerability in the apache default configurarion
...
Added a new WYSIWYG elementclass L<Krang::ElementClass::PoorText> and all
...
Fixed broken JS for ListGroup management in Edit Story UI. [Noam Weinstein]
