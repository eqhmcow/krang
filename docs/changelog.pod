...
objects with keys that may change (stories may change their URL, users
may change their login, etc.).  This allows updates between machines
to propogate these changes correctly when objects are moved with
krang_export and krang_import.  [Sam]

=item *

Added C<PasswordChangeTime> and C<PasswordChangeCount> directives
to allow the forcing of users to change their password evern C<PasswordChangeTime>
...
attempts. Configurable by the BadLoginCount and BadLoginWait conf
directives. [Michael Peters]
