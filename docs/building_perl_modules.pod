...
You must now add these, via cvs, to the project repository:
    ...
    find . -type f | grep -v CVS | xargs cvs add -ko
...
Make sure to update the B<$KRANG_ROOT/docs/cpan_inventory.pod> file.
This file contains a list of all the modules which have been
installed, listing their version and license.

If you are B<upgrading an existing module>, replace the reference to
the module in the list.  In this case, remove the old version of the
module from the repository via CVS.
