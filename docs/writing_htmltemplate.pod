...
With simple stories, it is easy to use a single template to handle the entire publish process.
...
Now let's imagine a more complex story.
...
the following variables will be made available: B<url>, B<title>
