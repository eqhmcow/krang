...
B<NOTE>: Any spaces in between words in the element's name will be replaced with underscores.

=item *

A single variable is created for every element, based on the element's name.  If there are several elements with the same name (e.g. multiple Paragraph elements), the value of the first one will be used.

An example would be <tmpl_var deck>.
...
This document only begins to cover what you can do in Krang.  If you are comfortable with programming in Perl, consider learning more about the construction and customization of elements and element libraries by reading B<INSERT DOCUMENT NAMES HERE>.
