...
SKIP: {
    ...
    # add five paragraphs
    ok($page->add_child(class => "paragraph", data => "bla1 " x 40), '$page->add_child(...)');
    ok($page->add_child(class => "paragraph", data => "bla2 " x 40), '$page->add_child(...)');
    ok($page->add_child(class => "paragraph", data => "bla3 " x 40), '$page->add_child(...)');
    ok($page->add_child(class => "paragraph", data => "bla4 " x 40), '$page->add_child(...)');
    ok($page->add_child(class => "paragraph", data => "bla5 " x 40), '$page->add_child(...)');
    ...
    is(
        ($element->match('/page[0]/paragraph[3]'))[0]->data(),
        "bla4 " x 40,
        '$element->match(...)->data'
    );
    is(
        ($element->match('/page[0]/paragraph[-1]'))[0]->data(),
        "bla5 " x 40,
        '$element->match(...)->data'
    );
    ...
    is($element->match('/page[1]/paragraph[6]'),
        0, '$element->match(...), number of matched children');
    is($element->match('/page[0]/paragraph[@data="' . ("bla4 " x 40) . '"]'),
        1, '$element->match(...), number of matched children');
    is(
        ($element->match('/page[0]/paragraph[@data="' . ("bla4 " x 40) . '"]'))[0],
...
END
    ...
    $element->child('page')->child('paragraph')->data('some new paragraph data...');
