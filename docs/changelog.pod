...
Fixed bug in Krang::ElementClass (probably introduced during implementation of 
flattened-template features for v3.04) that was causing publishing code to
fail to distinguish the empty-string from an undefined value when recursively
populating a template's inner loops. [Noam Weinstein]
