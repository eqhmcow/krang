...
is($@ =~ /Either the 'parent_id' or 'site_id' arg/, 1, "new() - missing 'site_id'");
...
my $category3 = pkg('Category')->new(
    dir       => '/bob3',
    parent_id => $parent->category_id
);
$category3->save();
my $category4 = pkg('Category')->new(
    dir       => '/bob4',
    parent_id => $parent->category_id
);
$category4->save();
my $category5 = pkg('Category')->new(
    dir       => '/bob5',
    parent_id => $parent->category_id
);
$category5->save();
...
# ids only
my @category_ids = pkg('Category')->find(
    ids_only => 1,
    url_like => '%bob%'
);
...
is(
    join(' ', @ancestors),
    $subcat->parent->category_id . ' ' . $subcat->parent->parent->category_id,
    'ancestors test'
);
...
is(
    ...
    $category->category_id . ' '
      ...
      . $category4->category_id . ' '
      . $category5->category_id . ' '
      . $subcat->category_id
);
...
sub test_linked_assets {
    ...
    my @test_stories;
    push @test_stories, pkg('Story')->new(
        class      => "article",
        title      => "title one",
        slug       => "slug one",
        categories => [$category]
