...
eval {
    my $siteX = pkg('Site')->new(
        preview_path => 'preview/path3/',
        preview_url  => 'preview.testsite3.t',
        url          => 'testfailure.t'
    );
};
like($@, qr/Required argument 'publish_path'/, "init() failure - publish_path");
...
$site3 = pkg('Site')->new(
    publish_path => 'pblish/path3/',
    preview_path => 'preview/path3/',
    preview_url  => 'preview.testsite3.t',
    url          => 'testsite3.t'
);
$site3->save();
my $site4 = pkg('Site')->new(
    publish_path => 'pblish/path4/',
    preview_path => 'preview/path4/',
    preview_url  => 'preview.testsite4.t',
    url          => 'testsite4.t'
);
$site4->save();
my $site5 = pkg('Site')->new(
    publish_path => 'pblish/path5/',
...
#
{
    ...
    my ($root_cat) = pkg('Category')->find(
        dir     => '/',
        site_id => $site->site_id
    );

    my $t1 = pkg('Template')->new(
        category => $root_cat,
