...
Added beta version of multiple-window, multiple-session support, allowing a user
in a single browser to open multiple windows, multiple stories, etc. In addition,
the user's login may be changed independently per window, allowing concurrent
users (for testing, training, admin, etc.) Needs fine-tuning! [Noam Weinstein]
...
Get C<bin/krang_install> ready for use with MySQL 5.* [Bodo Schulze]
...
C<--limited_to_category> option for C<bin/krang_publish>. [Bodo
Schulze]
