...
# constructor failure
my $tmpl;
eval { $tmpl = pkg('Template')->new(category => 'blah', content => 'blah', filename => 'A.tmpl') };
like($@, qr/'category' argument must be a 'Krang::Category'/s, 'constructor failure');

# constructor success 1 - tests category arg
eval {
    $tmpl = pkg('Template')->new(
        category => $category,
...
# Test permissions
{
    ...
    is($template->may_edit(), "0",
            "Can't edit template ("
          . $template->template_id
          . ") in read-only category ("
          . $test_cats[0]->category_id
          . ")");
    ...
    $template = pkg('Template')->new(
        category => $test_cats[0],
        filename => "noaccess\_$uniqueness\.tmpl"
    );
    eval { $template->save };
    isa_ok(
        $@,
        "Krang::Template::NoCategoryEditAccess",
        "Save to non-editable category " . $test_cats[0]->category_id . " throws exception"
    );
    ...
    ($template) = pkg('Template')->find(template_id => $test_templates[0]->template_id());
    is($template->may_edit(), "0",
            "Can't edit global template ("
          . $test_templates[0]->template_id()
          . ") w/ asset_template == 'read-only'");
    is($template->may_see(), "1",
            "Can see global template ("
          . $test_templates[0]->template_id()
          . ") w/ asset_template == 'read-only'");
    ...
    is($template->may_edit(), "0",
            "Can't edit global template ("
          . $test_templates[0]->template_id()
          . ") w/ asset_template == 'hide'");
    is($template->may_see(), "1",
            "Can still see global template ("
          . $test_templates[0]->template_id()
          . ") w/ asset_template == 'hide'");
    ...
    is($template->may_edit(), "0",
          ...
          . ") w/ asset_template == 'hide'");
    is($template->may_see(), "1",
        "Can see template (" . $test_templates[1]->template_id() . ") w/ asset_template == 'hide'");
