...
=item * Creating a Specialty Page

=over

=item * Example - Generating a Specialty First Page
...
=head1 Creating A Specialty Page

(Need further explanation of what a Specialty/Wall page is here)

To create a specialty page in Krang, you need to take the content that will make up the page (minus any header/footers that are added by the category), wrap it in delimiters (using a function provided by Krang), and include it in the HTML stream that makes up the full output from a published story.  During post-processing, Krang will take the specialty page from the block of HTML, wrap it in category headers/footers as needed, and write the page out to disk on its own accord.

The process by which the Specialty page is created is roughly as follows:

 my $html = $publish_results_to_this_point;

 my $specialty_html = generate_specialty_page();

 $html .= $publisher->begin_special_content() . $specialty_html . $publisher->end_special_content();

 return $html;


=head2 Example - Generating a Specialty First Page

In this case we have a story consisting of three pages, with the following element tree:
...
We want have two goals here - first, we want to build a three-page story out of this tree.  Second, we want to create a specialty page using the content of the first page, and a specialty template.

The best angle of attack here will be to work from the Story element - we want to manipulate content depending on which page element we're on, and page elements don't know about eachother.  Additionally, we need access to elements that won't be available to Page elements.

=head3 fill_template() - Story Element

In overriding the C<fill_template()> method in the story element, we're going to maintain two separate hashes of parameters - one will be used for the regular story template, the other for the specialty page template.
 ...
 sub fill_template {
     ...
     my $special_template = $self->_load_template(@_,
                                                  filename => 'special.tmpl',
                                                  search_path => ['/foo/bar']);
     ...
     $params{title} = $story->title();
     $special_params{title} = $story->title();
     ...
     foreach my $child (@children) {
         ...
         unless (exists($params{$name})) {
             $params{$name} = $html;
             $special_params{$name} = $html;
         }

         if ($name eq 'page') {
             push @{$params{"$name_loop"}}, { $name => $html };
             unless (exists($special_params{"$name_loop"})) {
                 push @{$special_params{"$name_loop"}}, { $name => $html };
             }
         }
     }

     $template->param(%params);
     $special_template->param(%special_params);

     my $html = $template->output();
     my $special_html = $special_template->output();

     $html .= $publisher->begin_special_content() . $special_html . $publisher->end_special_content();
