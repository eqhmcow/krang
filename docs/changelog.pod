...
(and for others, returned an arrayref!)  According to its own POD docs, 
...
these functions.  Krang now appears to return correct width and height values 
for all JPEG's tested. [David Kaufman]

=item * 

Separated the two perl classes in lib/Krang/FTP/FileHandle.pm into their own 
files so that either can be more easily subclassed in an addon, without having to 
