...
Media elements in Krang are all of the type Krang::ElementClass::MediaLink.  What this means is that while it inherits all the methods of Krang::ElementClass, it also provides a connection to Krang::Media objects.  
As a subclass of Krang::ElementClass, Krang::ElementClass::MediaLink objects use the same publish process, templates, overrideable methods and all.
...
The template for the image itself will be very small.  For example:


 <img src="<tmpl_var escape=html image_url>">
 <tmpl_if caption>
...
Generally, the default fill_template() method should be sufficient - but for reference purposes, overriding it would go as follows:
 ...
 sub fill_template {
    ...
    # fill in URL to link to media
    $template->param($self->name . "_url" => $media->url);
