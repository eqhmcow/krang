...
If that works, try building a database and running the test suite:

  make db test

If that worked, start up Krang and take a stroll through the UI:

  bin/krang_ctl start

Finally, you need to test installing a binary distribution.  First
make one:

  bin/krang_makedist

Then try installing it as usual.
