...
our %TEST_PACKAGES = map { ($_, 1) } (
    pkg('Alert'), pkg('Category'), pkg('Contrib'),   pkg('Desk'),
    pkg('Group'), pkg('List'),     pkg('ListGroup'), pkg('ListItem'),
    pkg('Media'), pkg('Site'),     pkg('Story'),     pkg('Template'),
    pkg('User')
);
...
sub check_find {
  ...
  SKIP: {
        ...
        ok(
            eq_array([@stuff], [reverse @stuff2]),
            "$perl_package->find(ids_only=>1, order_desc => 1) : order_desc=>1 reverses order_desc=>0"
        ) if @stuff;
    }
    ...
    ok(not($@),
        "$perl_package->find(offset=>1, limit=>1) : offset=>1, limit=>1 returns the next record");
    die($@) if ($@);

    my $order_by = $ORDER_BY_FIELD{$perl_package};
    if ($order_by) {
        is($stuff[1]->$order_by, $stuff2[0]->$order_by,
            "$perl_package->find(offset=>1, limit=>1) : offset=>1, limit=>1 returns the next record"
        ) if $stuff[1];
    }
    ...
    if ($order_by) {
        ...
        ok(not($@),
                "$perl_package->find( order_by => $order_by )"
              . ' : order_by $ORDER_BY_FIELD{$perl_package}');
    }
}
