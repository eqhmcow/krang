...
=head2 3.24 - October 21, 2013

=over

=item *

Preview URL returned by Krang::Publisher::preview_story is now the same as Krang::Story::preview_url [Michael Peters] 

=item *

Upgraded HTML::Template for bug fixes (2.91 -> 2.95) [Michael Peters]

=item * 

Fixed bug where sometimes ordering by URL when searching stories changed the results. [Michael Peters]

=item *

Fixed bug where tag information was lost on stories when editting the categories. [Michael Peters]

=item *

Scheduled jobs created after this release record the user_id for use in add_history. [Brad Oaks]

=item *

The history screen will show that an asset is published because of its link to another asset. [Brad Oaks]

=back

=head2 3.23 - July 31st, 2013

=over

=item *

Copying a story will now copy the tags. [Michael Peters]

=item *

Bug fix for scheduler crash: Deadlock found when trying to get lock. [Daniel S. Sterling]

=item *

Add run mode for help.pl to email the site administrator (C<BadHelpNotify>) about a missing help topic. [Brad Oaks]

=item *

Fixed a case where swapping story names could prevent files from being cleared when the story is retired. [Brad Oaks]

=item *

Fixed retired or trashed archive stories (having a story_category_link entry) being included for publish. [Brad Oaks]

=item *

Stories and Media skip the extraneous checkout/checkin history entries for (un)trash, (un)retire, and delete actions. [Brad Oaks]

=back

=head2 3.22 - October 18th, 2012

=over

=item *

Added "limit" option to Krang::Widget::autocomplete_values. [Michael Peters]

=item *

Added "Search Full Text?" checkbox on "Find Media" screen just like the
one on "Find Stories" screen. [Michael Peters]

=item *

Fixed bug where "Show Thumbnails" checkbox would become unchecked
on "Find Media" screen when a search was done. [Michael Peters]

=item *

Changed dates on messages about scheduled publishes to be a more user
friendly format [Michael Peters]

=item *

Added "Steal" capability to "Active Templates" screen. [Michael Peters]

=item *

Added "Steal" capability to "Active Media" screen. [Michael Peters]

=item *

Fixed bug where stealing stories from deleted users would result in an
ISE [Michael Peters]

=item *

Fixed bug where changing elements' classes and order at the same
time on Edit Story screens would garble data. [Michael Peters]

=item *

"Cancel" on edit screens is now multi-window friendly when editing
multiple objects in multiple screens. [Michael Peters]

=item *

Added "View Details" command to workspace screen. [Michael Peters]

=item *

Fixed Workspace sorting to work like normal krang pager style sortable
tables. [Michael Peters]

=back

=head2 3.21 - August 10th, 2012

=over

=item *

Added tags to stories for classification and searching. [Michael Peters]

=item *

Added tags to media for classification and searching. [Michael Peters]

=item *

New categories created with Media Bulk Upload will now have spaces
replaced by underscores to be consisten with how we also handle
filenames. [Michael Peters]

=item *

Consistently show the user's display name (first and last if we have
it) instead of login on different screens that refer to users by
name. [Michael Peters]
...
Fixed bug where New Template -> Save & Stay would hang and not
complete. [Michael Peters]

=item *

Fixed bug where Transform Image would try to operate on an image with
a known extension, but actually of the wrong type. [Michael Peters]

=item *

Fixed a bug where category choosers on New Media and New Template screens
didn't remember the last chosen category if it wasn't a "Save & Stay"
action. [Michael Peters]

=item *

Fixed a bug where switching between searching live and retired Media could
sometimes result in an ISE. [Michael Peters]

=item *

Fixed a bug where closing C<< </p> >> tags were being stripped during
Bulk Edit even when the separator wasn't C<< <p> >>. [Michael Peters]

=item *

Advanced search on Media filename no longer treats underscores as SQL
wildcard characters since they are really common in file names. [Michael
Peters]

=item *

Added new C<save_args> option to L<Krang::Story>'s C<transform_stories()>
method. [Michael Peters]

=back

=head2 3.20 - June 7th, 2012

=over

=item *

Adding full text search for text-based media objects (Javascript, HTML,
Stylesheets, Plain Text, etc). [Michael Peters]

=item *

Unload/Unlock the session in a post-run hook instead of waiting until the
full request is done (and the session object goes out of scope) so we don't
have to wait on clients to finish getting the request before we can start
handling another one. [Michael Peters]

=item *

Fixed bug where query string wasn't preserved when you had to login and it
tried to preserve the target destination after the login. [Michael Peters]
...
=head2 3.19 - April 13, 2012

=over

=item *

Don't convert hyphens in media filenames into underscores since hyphens
are valid in URLs and filenames. [Michael Peters]

=item *

Added C<no_anchor> option to L<Krang::Widget::autocomplete_values>.
[Michael Peters]

=item *

Fixing bug in template editor when code editor (CodePress) is on
that prevented edits after an initial "Save & Stay" from actually
saving. [Michael Peters]

=item *

After a successful login, if we were trying to get someplace else
originally redirect there instead of always taking the user to the
workspace. [Michael Peters]

=item *

L<Krang::FTP::Server> now logs properly in F<logs/krang.log>.
[Daniel S. Sterling]

=item *

L<Krang::FTP::Server> only call binmode utf8 for templates, not for media.
[Daniel S. Sterling]

=item *

C<story> is now passed to a top-level element's C<extension()> and
C<filename()> methods. [Michael Peters]

=item *

Fixed misspelling of F<krang_publish> C<--skip-related-assets> option.
[Michael Peters]

=back

=head2 3.18 - March, 08, 2012

=over

=item *

Templates can now be checked-out in bulk through the UI just like Stories
and Media. [Michael Peters]

=item *

Media Bulk Upload UI now remembers the last category that was used just
like the New Media screen. [Michael Peters]

=item *

Searching for media by filename no longer wild cards non-word characters
since those are important in file names (like ".") [Michael Peters]

=item *

Templates that are deployed from the "Find Templates" screen are
now checked-in if they were checked-out by the person doing the
deployings. This way deploy from "Find Templates" works the same as
deploy from "Edit Template" [Michael Peters]

=item *

Publisher now looks at all the categories for a story in the
C<_add_category_linked_stories()> method. [Brad Oaks]

=item *

Deleting or retiring a story in the UI will now redirect to publish_story_list
if category linked stories need published. [Brad Oaks]

=item *

The Scheduler now checks all instances on startup for jobs with
daemon_uuid reservations that match our hostname.
[Daniel S. Sterling]

=item *

Added Fedora 15 & 16 as build platforms. [Michael Peters]

=item *

L<Krang::HTMLPager> now adds double-backslash-escaping to C<&#39;> and C<&apos;>
entities in the C<command_column_extra_args> for dynamic columns. [Brad Oaks]

=back

=head2 3.17 - February, 06, 2012

=over

=item *

Fixed bug in which the krang_install script did not respect the DBSock
conf file option, preventing installation and upgrade when using a 
nonstandard mysql socket location
[Rudolf Lippan]

=item *

The Scheduler now makes sure jobs either die or call clean_entry
or delete.  If they don't do one of those three things before returning,
we log an error.  This makes sure next_run is always updated.
[Daniel S. Sterling]

=item *

Added a method to ElementClass which can be overridden to decide
during a publish if a L<Krang::ElementClass::CategoryLink> should
cause other stories to be published. [Brad Oaks]

=back

=head2 3.16 - January, 25 2012

=over

=item *

On startup the C<bin/krang_scheduled> daemon clears the claim for any
stuck Schedule entries where the claim contains the machine's hostname.
[Brad Oaks]

=item *

Updated C<Krang.Window.log_out()> so that it updates other logged in
windows or tabs to reflect that the user has logged out. [Brad Oaks]

=item *

Optimize C<Krang::Group->add_user_permissions()> so that it doesn't
perform so many database queries. The count can grow dramatically as
the number of users and categories increases. [Michael Peters]

=item *

Added C<--groups> option to F<bin/krang_floodfill>. [Michael Peters]

=item *

Fixed the "Edit Media" screen to be able to transform images in multiple
windows.  [Brad Oaks]

=item *

Prevent users from submitting 2 different actions at the same time when
editting elements (eg, Add Element + Reorder) before the 1st one has
finished. [Michael Peters]

=item *

Fixed element editor bug where re-ordering and adding elements at
the same time could result in weirdness depending on the element's
classes. [Michael Peters]

=item *

C<Krang::ElementClass::TopLevel::publish_frontend_app_template()> now has
an additional (optional) C<template> parameter that can be used in place
of C<filename> if you already have a template to pass. [Michael Peters]

=item *

Fixing bug where messages during publish were getting lost [Michael
Peters]

=item * 

Allowed multiple schedulers to run without processing the same
schedules. [Bodo Schulze]

=item *

Added 4 flags to L<Krang::ElementClass::CategoryLink> implementing a
'conditional publishing behavor' for stories having CategoryLinks.
If this behavior is set via any 4 flags, then this story can become a
publish dependency of other stories/media in those linked categories.
[Bodo Schulze]

=back

=head2 3.15 - November, 16 2011

=over

=item *

Publishing through the UI no longer locks the session while the publishing
is happening. [Michael Peters]

=item *

Adding C<--no-ssl-symlinks> option to Krang upgrade to skip rebuilding
the SSL cert symlinks. [Michael Peters]

=item *

Added a new C<session_id> method to L<Krang::Session> to make it really
easy to determine the current session's ID. [Michael Peters]

=item *

Fixed story_find help text. [Bodo Schulze]

=item *

Added F<docs/feature_history> to document new features and functional
enhancements from a user perspective. [Bodo Schulze]

=item *

Put back session locking because without it there are race conditions
when we deal with multiple windows and nested data structures in the
sessions. [Michael Peters]

=item *

Allow IFRAME tags in Xinha. [Bodo Schulze]

=item *

Auto-preview media objects that are directly uploaded to a story so
that they will already be ready when the story itself is previewed.
[Michael Peters]

=item *

C<Krang.Ajax.update()> javascript function can now take a function as
the value of C<target> or C<to_top> to be executed on-demand after the
actual request is done. [Michael Peters]

=item *

The second argument to the SQL 'limit' clause must no be a negative
number.  Instead use a very high positive number in the find() method
of backend modules. See
http://http://dev.mysql.com/doc/refman/5.5/en/select.html. [Bodo
Schulze]

=item *

Added 'contribs' param to create_story() and create_media() in
L<Krang::Test::Content>. [Bodo Schulze]

=item *

Fixed upgrade module V3_10 in tags 3.10 - 3.14 and on the trunk. [Bodo
Schulze]

=item *

Fixed problem with F<bin/krang_apachectl> not stopping an running Krang
in Offline Mode [Michael Peters]

=item *

Fixed problem with ISEs resulting from POST requests causing problems with
the ErrorHandler and not giving the same response as GET ISEs. [Michael
Peters]

=item * 

Changed multi-window feature to use a single session per-login instead
of using a different session for each window. [Michael Peters]

=item * 

Fixed ISE (regression) that occurs when you try an advanced search for
retired stories. [Michael Peters]

=item *

Added exception handling for edge cases to
F<bin/krang_charset_convert_data>. [Bodo Schulze]

=item *

Added C<--keep-sessions> option to F<bin/krang_upgrade>. [Michael Peters]

=item * 

Fixed an ISE where stories with duplicate URLs as categories were not
caught and shown to the user. [Michael Peters]

=back

=head2 3.14 - June, 23 2011

=over

=item *

Reset environment variables set by L<ExtUtils::MakeMaker> and
L<Module::Build> to get a clean start. [Bodo Schulze]

=item *

Added script F<bin/krang_storable_freezethaw> to help migrating between
32bit and 64bit architectures. [Bodo Schulze]

=item *

Added C<IgnorePreviewRelatedStoryAssets> and
C<IgnorePreviewRelatedMediaAssets> configuration directives so that
instances can choose to ignore publishing related assets during
preview. [Michael Peters]

=item *

Removed the need for F<bin/krang_addon_dist> to have a valid F<krang.conf>
since it doesn't actually use anything in that. This allows for
automating the building of a package from a Krang svn checkout that is
built. [Michael Peters]

=item *

Fixed FTP download of templates with UTF8 characters in binary mode.
[Brad Oaks]

=item *

Editing and saving an existing category will now convert a story with
a duplicate url into a category index. This check already happens when
creating a new category.  [Brad Oaks]

=item *

The Find Story button for a Lead In element of a story now works in the
Chrome browser.  [Brad Oaks]

=item * 

When Bulk Editting something with a C<< <p> >> tag as a separator,
we now strip off any remaining stray closing C<< </p> >> tags as
well. [Michael Peters]

=item *

When someone steals a story or template from a user, give a more
descriptive error message including the name of the person who stole
it. [Michael Peters]

=item *

Advanced "Find Story" screen will now find stories that are in the
category the user specifies even if it isn't the story's primary
category. Otherwise it can be impossible to find the story you're looking
for. [Michael Peters]

=back

=head2 3.13 - Apr, 5 2011

=over

=item *

Fixed display issue of time selector in Krang::ElementClass::DateTime
(thanks to Mike Lampson for this patch). [Bodo Schulze]

=item *

Fixed bug where you couldn't uncheck the "Show Story Type and Version"
checkbox on the "Find Stories" screen. [Michael Peters]

=item *

Fixed bug where you couldn't uncheck the "Search full text?" checkbox
on the "Find Templates" screen. [Michael Peters]

=item *

C<hidden> parameters of elements can now be a subroutine reference which
will be executed at runtime and that value used. [Michael Peters]

=item *

Fixed problems with un-localized C<$@> which could make some error
messages not appear in the logs. [Michael Peters]

=item *

Fixed wacky partial error screen when unexpected erorrs happened during
publish from the web UI. [Michael Peters]

=back

=head2 3.12 - Jan, 13 2011

=over

=item *

Added C<--NoStart> option to F<bin/krang_install> when you want to install
but then later tweek things (like F<conf/krang.conf>) before starting the
servers. [Michael Peters]

=item *

Added C<--NoDB> option to F<bin/krang_install> for use when you already
have an existing Krang DB and want a new install to use that. [Michael Peters]

=item *

Added "hostname dictionary" feature to F<krang.conf> so that you can
specify different values for configuration directives per-hostname.
This allows multiple machines in a Krang cluster to share the same
F<krang.conf> file. [Michael Peters]

=item *

Fix bug in F<krang_ctl> that incorrectly used C<sudo> when
starting/stopping the scheduler. [Michael Peters]

=item *

Prevent media files from getting spaces (or other forbidden characters)
in their filenames when bulk-uploaded or added via FTP (already forbidden
in the web UI). [Michael Peters]

=item *

Updated accidental limit where publisher couldn't handle story_ids over
4194304. [Mike Lampson & Michael Peters]

=item *

Don't delete temporary expanded XML files when a KDS import fails so it
can be inspected. [Michael Peters]

=item *

Fix problem of starting Apache still being attached to the IO of the starting
terminal making it hang SSH sessions. [Michael Peters]

=item *

Fix bug of F<krang_ftpd> where the new FTP daemon wasn't always detaching
itself from the terminal process that started it, thus hanging SSH
sessions when trying to disconnect.

=item *

Fixed bug in IE where people randomly lose their session if they are doing
a lot of publishing and previewing. [Michael Peters]

=item *

Hitting "Save & Stay" on the "Edit Template" screen now doesn't reset the
template editor, so you don't lose your place. [Michael Peters]

=item *

Bulk Upload of Media can now handle resizing images with .jpg and .jpeg
extensions in addition to .png, .gif, and .tiff files.  [Brad Oaks]

=back

=head2 3.11 - August 30, 2010

=over

=item * 

Added C<DBIgnoreVersion> configuration directive to allow for times when
you're upgrading multiple Krang machines at the same time while others
are running. [Michael Peters]

=item *

Pager page numbers and count are now formatted numbers [Michael Peters]

=item *

Status page now uses the C<Charset> directive like the rest of Krang. [Michael Peters]

=item *

Improved performance of ISE notification by running the code under mod_perl
and not CGI. [Michael Peters]

=item * 

Added C<--profile-mode> option to turn on profiling with L<Devel::NYTProf>
from F<bin/krang_apachectl>. [Michael Peters]

=item *

Allow CGI mode to be activated with C<--cgi-mode> option to
F<bin/krang_apachectl>. [Michael Peters]

=item *

Improved navigation rendering performance. [Michael Peters]

=back

=head2 3.10 - June 29, 2010

=over

=item *

Searching for a story now normalizes the url so it can be compared
without problems. [Michael Peters]

=item *

Added storage of C<krang_pager_curr_page_num> and
C<krang_pager_show_big_view> to session cache in In
L<Krang::HTMLPager>. The page number is forced to C<1> when switching
between Retired or Live templates, media, and stories. [Brad Oaks]

=item *

Searching for media on the advanced tab will now return media of all
types when no media_type_id is selected. [Brad Oaks]

=item *

Added C<--skip-media> and C<--skip-stories> options to
F<bin/krang_publish>. [Michael Peters]

=item *

Fixed bug that prevented new categories from being created with the same
URL as an existing story in some circumstances. [Michael Peters]

=item * 

Some autocomplete values were coming back as all lower-case. Case is
now preserved. [Michael Peters]

=item *

Krang scheduler can now be disabled per-instance with the DisableScheduler
directive set to 1. [Brad Oaks]

=item *

Krang will now correctly guess the media type when media files are uploaded
via the FTP interface. [Michael Peters]

=item *

When bulk editing a Story, the C<Done With Bulk Edit> button is now working.  [Brad Oaks]

=item *

Alerts are now able to be sent for actions on any Story or any Media instead of only on specific ones.  [Brad Oaks]

=item *

C<ElementClass::PopupMenu> will now execute subroutine references which are passed to new() for the C<value> key.  [Brad Oaks]

=item *

Importing stories has been greatly improved in the following
ways. [Michael Peters]

=over

=item * Circular dependencies between categories and stories are now handled better

=item * Stories that are changing their class in the import are now handled

=item * When checking for stories who's non-primary url might conflict, we correctly ignore the imported story's other URLs

=back

=back

=head2 3.09 - March 08, 2010

=over

=item *

Adding C<ReservedURLs> configuration directive.  This prevents Krang
users from creating stories or categories with certain URLs.  This is
useful when you have another application serving the Krang published
content and it needs certain URLs reserved for it's functionality.
This can be set per-instance or globally. [Michael Peters]

=item *

Fixed ISE that occurred when certain bad date formats were used
for Cover Dates on stories. [Michael Peters]

=item *

Upgraded L<Apache::Session> and changed our usage to not require
an exclusive lock for every request. [Michael Peters]

=item *

Added the F<bin/krang_generate_csv> script. [Michael Peters]

=item *

We now generate higher quality thumbnails and there were a couple of
places (like "Select Media") where thumbnails weren't showing up when
they should have. [Michael Peters]

=item * 

Added new C<command_column_extra_args> option to L<Krang::HTMLPager>
so that more arguments can be passed into the C<command_column_commands>
besides just the id. [Michael Peters]

=item *

Added C<read_only> flag to Media and Template objects. This flag
can only be set by the F<bin/krang_read_only> admin script so that these
items can't be set/unset by users. [Michael Peters]

=item *

Added C<--category-links> option to F<bin/krang_create_story>
script. [Michael Peters]

=item *

Media Bulk Upload can set a max height and/or max width to scale down
large images. [Brad Oaks]

=item *

Added C<Krang.Form.get_form()> method to make it easier to get the
form in custom Javascript code. [Michael Peters]

=item *

Added C<update> (which defaults to C<true>) option to C<Krang.Form.submit()>
to make it easier for addons to create forms which submit and return JSON data. [Michael Peters]

=item *

Added ability to search Media by C<Type> (which is C<media_type_id>). [Brad Oaks]

=item *

Fixed ISE that occurs on the Back button when viewing an older version of a template that you have checked out.
It only happened when starting from Workspace and editing a template.  The form action was set to workspace.pl
instead of template.pl in that case.  [Brad Oaks]

=back

=head2 3.08 - December 20, 2009

=over

=item *

Media items now call the element's C<retire_hook>. [Michael Peters]

=item *

Added C<may_view_trash> permission. [Michael Peters]

=item *

Fixed ISEs that occurs if you try to check out media or templates
that were checked out by someone else while you were looking at
(normally prevented by the UI). Now you get a helpful alert message
instead. [Michael Peters]

=item *

Fixed ISE on "New Media" screen when submitting the form via "enter"
instead of clicking the "Create" button. [Michael Peters]

=item *

Going from Trash -> View Detail -> View Log -> Back for Media objects
no longer results in an ISE. [Michael Peters]

=item *

Catch more kinds of template errors when previewing a story so they
don't result in an ISE, but a better error message. [Michael Peters]

=item *

Added Fedora 12 as a build target. [Michael Peters]

=back

=head2 3.07 - November 5, 2009

=over

=item * 

Fixed various failing tests. [Michael Peters]

=item *

Fixed problem with URL wrapping that prevented copy-pasting of those URLs
from the UI. [Michael Peters]

=item *

Adding C<MinSpareServers>, C<MaxSpareServers>, C<MaxClients> F<krang.conf>
directives that can be used to control how many running Apache processes there
are. [Michael Peters]

=item *

Fixed JS error on Transform Image screen when cancelling and going to
another page. [Michael Peters]

=item *

Fixed IE8 where the Category Chooser on Edit Media screen
was hidden behind every table but the initial one (the meta-data
table). [Michael Peters]

=back

=head2 3.06 - October 18, 2009

=over

=item *

Adding a C<--no-upgrade> option to F<bin/krang_addon_installer> to
make it easier to manage in a cluster. [Michael Peters]

=item *

Unknown publish errors from the Web UI now result in a normal ISE
(die) like other errors and are displayed with the same modal dialog
in the UI. This also triggers the C<ErrorNotificationEmail> to be sent
if configured. [Michael Peters]

=item *

Fixed error when publishing a MediaLink element that didn't actually
have a media object selected. Now we just silently return and continue
with the publish. [Michael Peters]

=item *

Normalize Unicode characters coming in so that we always have single
grapheme characters. Unicode can have "composed" characters where the
same character is actually represented by combining multiple characters.
Use NFC normalization so that we just store a single character. This will
make it easier to do things like full text search on Unicode characters
that might have been submitted as multiple graphemes. [Michael Peters]

=item *

Fixed IE7 where the Category Chooser on Edit Media screen
was hidden behind every table but the initial one (the meta-data
table). [Michael Peters]
