...
That's all there is to producing a simple Krang add-on.  Just lay-out
your files as you wish them to be in Krang, put them in a directory
with the format "$name-$version" and make a .tar.gz.  The rest of this
document will explain enchancements that can be used to create more
complicated add-ons.
...
You can declare the name of your add-on.  If you do not declare the
name here it will be parsed from the name of your add-on directory
(thus Turbo-1.00 has the name 'Turbo').  Names must match
C</^[-\w]$/>.  For example:

  Name Turbo

=item * C<Version> I<x.y>

You can declare the version of your add-on.  If you do not declare the
version here it will be parsed from the name of your add-on directory
(thus Turbo-1.00 is assumed to be version 1.00).  A version must be a
...
add-on installation) are installed into Krang.  For example,
Turbo-1.00 might list:
...
will be set in the environment for this script.  Note that this script
is run for installation, not for upgrades.  See L<"Upgrades"> for
information on scripting upgrades.
...
'upgrade/V1.010.pm' must be created (note the required expansion to
three decimal places).  This file may contain code to run on a
per-installation or per-instance basis, just like other Krang upgrade
scripts.

Krang's upgrade process is described in the 
HREF[Operations Upgrade|ops_upgrade.html] document.
